...
transit_data_distinct_reformat = 
  read_csv(file = "./data/NYC_Transit_Subway_Entrance_And_Exit_Data.csv") %>% 
  janitor::clean_names() %>% 
  gather(key = "route", value = "route_number", route1:route11) %>%
  separate(route, into = c("route_name_str","route_number_str","route"), 
           sep = 5) %>% 
...
summarize(brfss_smart2010, median(excellent, year = 2002, na.rm = TRUE))
